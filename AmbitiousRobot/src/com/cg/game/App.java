...
public class App extends JFrame implements KeyListener, ActionListener {
    ...
    public App(Inicio i) throws HeadlessException {
        ...
        baby = new Robot(new Point(120, 480));
        baby.cargarImagenes();
    ...
    public synchronized void crearObjetos() {
        ...
        if (tipo < 0.45) {
            objetos.add(new ElementoDanino(e));
        } else {
            if (tipo < 0.9) {
                objetos.add(new ObjetoDeValor(e));
            } else {
                objetos.add(new Lingote(e));
            }
        }
    ...
    public void muerte() {
        if (i.getPlayer().getVida() <= 0) {
            ...
            ene.stop();
            JOptionPane.showMessageDialog(null, "Game over" + i.getPlayer().getNombre() + " \n Your score is: " + puntos);
